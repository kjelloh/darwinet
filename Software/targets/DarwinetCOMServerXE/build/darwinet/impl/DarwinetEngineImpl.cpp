...
namespace darwinet {
	...
	namespace miv {
		...
		/**
		  ...
		  */
		void c_MIVValueInstanceImpl::addListener(c_MIVValueInstanceListener* pListener) {
			if ((this->m_pListener != NULL) && (this->m_pListener != pListener)) {
				c_LogString sMessage(__FUNCTION__" called with new Listener. Multipple listeners not yet implemented. Old listener will be replaced by new one");
		...
		/**
		  ...
		  */
		void c_MIVValueInstanceImpl::removeListener(c_MIVValueInstanceListener* pListener) {
			if ((pListener != NULL) && (pListener != this->m_pListener)) {
				c_LogString sMessage(__FUNCTION__" called with unregistered Listener. Multipple listeners not yet implemented. Listener reference not changed and other listener may remain.");
				LOG_DESIGN_INSUFFICIENCY(sMessage);
			}
			else {
				this->m_pListener = NULL; // No listener
			}
        }
		...
		/**
		  ...
		  */
		void c_MIVValueInstanceImpl::reportValueChanged() {
			if (this->m_pListener != NULL) {
				this->m_pListener->actOnMIVValueInstanceValueChanged();
			}
		}
