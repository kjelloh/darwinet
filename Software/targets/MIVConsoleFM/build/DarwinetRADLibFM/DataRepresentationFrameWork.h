...
/*
	...
	All string representation change is made with explcit call to toXXXString() methods.
...
/**
  ...
  * representations.
  */
class PACKAGE c_DataRepresentationFramework {
public:
	...
	public:
		// Allow implicit construct from anonymous string
	...
	/**
	  ...
	  */
	static c_DataRepresentationFramework::c_UTF8String& toUTF8String(const c_DataRepresentationFramework::c_UTF8String& sUT8String);
...
//		s.erase(s.begin(), std::find_if(s.begin(), s.end(), std::not1(std::ptr_fun<int, int>(std::isspace))));
		s.erase(s.begin(), std::find_if(s.begin(), s.end(), std::not1(std::ptr_fun(std::isspace)))); // bcc64 compiler generates  "'(' for function-style cast or type construction" if std::ptr_fun is defined with template parameters.
		return s;
	}
...
//		s.erase(std::find_if(s.rbegin(), s.rend(), std::not1(std::ptr_fun<int, int>(std::isspace))).base(), s.end());
		s.erase(std::find_if(s.rbegin(), s.rend(), std::not1(std::ptr_fun(std::isspace))).base(), s.end()); // bcc64 compiler generates  "'(' for function-style cast or type construction" if std::ptr_fun is defined with template parameters.
		return s;
	}
