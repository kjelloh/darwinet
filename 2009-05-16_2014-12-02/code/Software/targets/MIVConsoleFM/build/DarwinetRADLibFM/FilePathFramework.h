...
public:
	...
	/**
	  ...
	  * I.e the part of the name before the last '.'
	  */
...
public:
	...
	typedef boost::shared_ptr<c_FilePathNodeArray<N> > shared_ptr;
...
//	c_FilePathNodeArray(const c_FilePathNodeArray<N>& other_value)
	...
	/**
	  * Asignment operator
	  */
	c_FilePathNodeArray<N>& operator=(c_FilePathNodeArray<N> other_value_copy) {
//		c_FilePathNodeArray<N> temp_copy(other_value);
		static_cast<std::vector<N>&>(*this).swap(other_value_copy);
		return *this;
	}
...
private:
	...
	void buildArrayFromString(const S& sFilePath) {
		try {
			...
			// Start by checking if there is a preceeding server node, one started with back-slash
			...
			if (*iter == '\\') {
				if ((iter+1) != sFilePath.end()) {
					if (*(iter+1) == '\\') {
						...
						node_caption = _Asciisz("\\\\"); // Server node caption includes the back-slash
					}
				}
			}
...
/**
  ...
  */
template <class _FilePath>
class c_FilePathFrameworkT {
public:

	// shared pointer to instance of path of type _FilePath
	typedef boost::shared_ptr<_FilePath> shared_ptr;

	/**
	  * Conveniant typedef
	  */
	typedef typename _FilePath::node_type N;

	/**
	  * Creates provided directory path in file system.
	  * Each node in the path will be created if it does not yet exist
	  */
	static bool directoryPathExists(const _FilePath& directory_path) {
		bool result = false; // default
		result = DirectoryExists(directory_path.toString().c_str());
		return result;
	}

	typedef std::vector<shared_ptr> c_FilePathList;

	/**
	  * Returns a list of all file paths of provided direcory path.
	  */
	static c_FilePathList getDirectoryList(const _FilePath& directory_path) {
		c_FilePathList result;
		// Use boost/filesystem to get the directory listing
		boost::filesystem::wpath boost_directory_path(directory_path.toString().c_str());
		if (boost::filesystem::exists(boost_directory_path)) {
			// OK. Is it a directory
			if (boost::filesystem::is_directory(boost_directory_path)) {
				// Sure :)

				// boost version > 1.46 uses BOOST_FILESYSTEM_VERSION 3.
				// Filesystem3 has only one 'path' type (not wide- or narrow-character variations)
				// https://github.com/rowanj/LucenePlusPlus/commit/5d0117d1720a52e49149be67b7095cc0bd4adb32
#if (BOOST_FILESYSTEM_VERSION > 2)
				boost::filesystem::directory_iterator iter(boost_directory_path);
				boost::filesystem::directory_iterator iter_end;
				while (iter != iter_end) {
					boost::shared_ptr<_FilePath> pPath(new _FilePath(iter->path()));
					result.push_back(pPath);
					++iter;
				}
#else
				boost::filesystem::wdirectory_iterator iter(boost_directory_path);
				boost::filesystem::wdirectory_iterator iter_end;
				while (iter != iter_end) {
					boost::shared_ptr<_FilePath> pPath(new _FilePath(iter->path()));
					result.push_back(pPath);
					++iter;
	...
	/**
	  ...
	  * Each node in the path will be created if it does not yet exist
	  */
	static bool ensureDirectoryPathExists(const _FilePath& directory_path) {
		bool result = false; // default

		// Loop through the path and ensure that each directory node exists
		typename _FilePath::const_iterator iter = directory_path.begin();
		_FilePath current_path;
...
//						c_LogString sMessage(__FUNCTION__" failed. CreateDir() returned error ");
						c_LogString sMessage = METHOD_NAME + c_LogString(" failed. CreateDir() returned error ");
	...
	/**
	  ...
	  * in the file system.
	  */
	static bool fileExists(const _FilePath& file_path) {
		bool result = false; // default
	...
	/**
	  ...
	  */
	static void deleteFile(const _FilePath& file_path) {
	...
	/**
	  ...
	  */
	static void renameFile(const _FilePath& folderPath,const N& oldFileName,const N& newFileName) {
		_FilePath old_file_name_path = folderPath;
		old_file_name_path += oldFileName;
		_FilePath new_file_name_path = folderPath;
	...
	/**
	  ...
	  */
	static bool copyFile(const _FilePath& source_path,const _FilePath& target_path) {
		bool result = false; // default
	...
	//	#define COMPILE_UNALLOWED_CONSTRUCTS // DOcompile constructs that must generate compile errors
...

		{
			...
			// 141121, bcc64 is unable to implitly convert _XXXsz to c_XXXString to use c_XXXFilePath(c_XXXString). Why?
			//         E.g., bcc64 is unable to implitly convert _UTF8sz to c_UTF8String to use c_UTF8FilePath(c_UTF8String).
			//         Solve by giving is c_XXXString directly.

//			c_UTF8FilePath UTF8FilePath = _UTF8sz("\\Mapp1\\Mapp med ö\\Filnamn med ö.txt");
			c_DataRepresentationFramework::c_UTF8String sUTF8FilePath = _UTF8sz("\\Mapp1\\Mapp med ö\\Filnamn med ö.txt");
			c_UTF8FilePath UTF8FilePath = sUTF8FilePath;
...
//			c_FileName Filename2 = c_FileName(u"Mitt filnamn 2 med ö.txt");
			c_FilePath FilePath1 = c_FilePath::string_type(u"\\Mapp1\\Mapp med ö\\Filnamn med ö.txt");
			c_FilePath FilePath2 = c_FilePath::string_type(u"\\Mapp2\\Mapp med ö\\Filnamn med ö.txt");
			c_FileName Filename1 = c_FileName(u"Mitt filnamn 1 med ö.txt");
			c_FileName Filename2 = c_FileName(u"Mitt filnamn 2 med ö.txt");
