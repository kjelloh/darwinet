...
/**
  * Models an instance that may be used to monitor
  * data created on the heap.
  */
class c_HeapSupervisor {
public:
	typedef boost::shared_ptr<c_HeapSupervisor> shared_ptr;
	...
	/**
	  ...
	  */
	virtual void startMonitoring(void* pObject,int size_in_bytes) = 0;

	/**
	  * Removes provided object adress from the list of heap locations to minitor
	  */
	virtual void stopMonitoring(void* pObject) = 0;

	/**
	  * Will log any changes to monitored objects
	  */
	virtual void logMonitoredObjectState() = 0;
...
/**
  ...
  */
extern PACKAGE c_LogString toLogString(const c_DataRepresentationFramework::c_UTF16String& sEntry);
...
/**
  * Converts provided C++ literal string to Log string representation
  */
extern PACKAGE c_LogString toLogString(const c_DataRepresentationFramework::c_ISO_8859_1_String& sEntry);
...
/**
  ...
  */
extern PACKAGE c_LogString toLogString(const System::UnicodeString& sEntry);

///**
//  * Converts RAD Studio XE System::AnsiString to c_LogString
//  * Note: RAD Studio XE type String may be this type depedning on project settings.
//  */
//extern PACKAGE c_LogString toLogString(const System::AnsiString& sEntry);

/**
  * Returns a log string with a text describing the menaing of provided Windows api error code.
  */
extern PACKAGE c_LogString logStringOfWinApiErrorCode(DWORD api_error_code);

/**
  * Returns provided Windows HRESULKT code as a descriptive Log string
  */
extern PACKAGE c_LogString logStringOfWinApiHResultCode(HRESULT hresult);
...
/**
  ...
  */
class PACKAGE c_BussinessLoggerIfc {
public:
...
//#ifdef __FUNCTION__
//	#define METHOD_NAME c_LogString(__FUNCTION__)
//#else
//	#define METHOD_NAME c_LogString(__func__)
//#endif

// 141123, Standard c++ would require typedid(class).name() to get the class name (although it may not be human readable)
//         Standrad __func__ are not required to show class_name::method_name for class methods.
//         In bcc64 __func__ seems to retun only the function name, not the class for member functions.
//	       typeid(*this).name() works ony for non-static class members (i.e. functions having an implicit this parameter)
//#define METHOD_NAME c_LogString(c_LogString(typeid(*this).name()) + c_LogString(BOOST_CURRENT_FUNCTION))
#define METHOD_NAME c_LogString(BOOST_CURRENT_FUNCTION)


// Insetad of (__FUNCTION__" Bla bla bla...")
#define METHOD_PREFIXED_LOG_STRING(S) METHOD_NAME + c_LogString(S)
...
//	#define LOG_SCOPE_ENTRY(S) c_BussinessLoggerIfc::instance()->LogScopeEntry(S)
	...
	#define LOG_FUNCTION_SCOPE\
		...
		c_LogString sFunctionEntryString(METHOD_NAME);\
	...
	#define LOG_METHOD_SCOPE \
		...
		/*sFunctionEntryString += _UTF8sz("::"__FUNCTION__);*/\
		sFunctionEntryString += c_LogString("::") + METHOD_NAME;\
		c_DevelopmentLogTraceIndentator DevelopmentLogTraceIndentator(sFunctionEntryString);

	#define LOG_SCOPE_S(S) c_DevelopmentLogTraceIndentator DevelopmentLogTraceIndentator(S);
	#define LOG_FUNCTION_SCOPE_S(S) \
		/*sFunctionEntryString += _UTF8sz("::"__FUNCTION__"(");*/\
		c_LogString sFunctionEntryString = c_LogString("::") + METHOD_PREFIXED_LOG_STRING("(");\
	...
	#define LOG_METHOD_SCOPE_S(S) \
		c_LogString sFunctionEntryString(c_DataRepresentationFramework::intToHexString(reinterpret_cast<unsigned int>(this))); \
		/*sFunctionEntryString += _UTF8sz("::"__FUNCTION__"(");*/\
		sFunctionEntryString += c_LogString("::") + METHOD_PREFIXED_LOG_STRING("(");\
		sFunctionEntryString += toLogString(S); \
		sFunctionEntryString += _UTF8sz(")"); \
		c_DevelopmentLogTraceIndentator DevelopmentLogTraceIndentator(sFunctionEntryString);
	/**
...
#define LOG_IDE_EXCEPTION_DESIGN_INSUFFICIENCY \
		...
		c_LogString sMessage(METHOD_PREFIXED_LOG_STRING(" failed. Exception="));\
...
#define LOG_GENERAL_EXCEPTION_DESIGN_INSUFFICIENCY \
		...
		c_LogString sMessage(METHOD_PREFIXED_LOG_STRING(" failed. General Exception cought."));\
		LOG_DESIGN_INSUFFICIENCY(sMessage)
