...
__fastcall TEditApplicationPropertiesFrame::TEditApplicationPropertiesFrame(TComponent* Owner,c_EditApplicationPropertiesFrameDecoratorIfc* pEditApplicationPropertiesFrameDecoratorIfc)
	: TFrame(Owner)
//	  ,m_pTestBenchPropertiesModel(new c_ApplicationPropertiesModel(_UTF16sz(Application->ExeName.c_str())))
//	  ,m_pTestBenchPropertiesModel(new c_ApplicationPropertiesModel(_UTF16sz(L""))) // Empty anonymous propery instance
	  ,m_pTestBenchPropertiesModel(new c_ApplicationPropertiesModel(c_FileName(u""))) // Empty anonymous propery instance
	  ,m_pEditApplicationPropertiesFrameDecoratorIfc(pEditApplicationPropertiesFrameDecoratorIfc)
{
	...
	this->Name = sName;
	m_pTestBenchPropertiesModel->addListener(this);
}
//---------------------------------------------------------------------------
...
void TEditApplicationPropertiesFrame::buildPropertiesEditor(TStringList* pPropertiesList,TStringList* pObsoletePropertyList) {
	...
	this->m_pTestBenchPropertiesModel->setObsoleteProperties(pObsoletePropertyList);
	...
	if (row_index > this->ValueListEditor1->RowCount) {
		...
		if (this->isFilePathProperty(name)) {
			...
			if (this->OpenDialog1->Execute()) {
				...
				this->getTestBenchPropertiesModel()->setPropertyValue(name,OpenDialog1->FileName);
			}
		}
	}
}
...
//	else if (KeyName == "SPDH.Router.Client.ACISTD.LengthField") {
//		// Add a drop down property editor for this value
//		Values->Add("As_Host");
//		Values->Add("Two_bytes_self_including");
//		Values->Add("Two_bytes_self_excluding");
//		Values->Add("No_length_field");
//	}
//	else if (KeyName == "SPDH.Router.Client.Parity") {
//		Values->Add("As_Host");
//		Values->Add("Even");
//		Values->Add("No_Parity");
//	}
//	else if (KeyName == "Testbench.iPOSTerminal.ConnectionType") {
//		Values->Add("TCP/IP");
//		Values->Add("RS232");
//		Values->Add("USB");
//		Values->Add("Other");
//	}
//	else if (KeyName == "Testbench.SaleEnvironmentType") {
//		Values->Add("Retail");
//		Values->Add("PetrolDispenser");
//	}
...
/**
  * Returns the test bench properties model used by this frame.
  * Changes to the model will update the frame.
  */
//c_ApplicationPropertiesModel* TEditApplicationPropertiesFrame::getTestBenchPropertiesModel() {
boost::shared_ptr<c_ApplicationPropertiesModel> TEditApplicationPropertiesFrame::getTestBenchPropertiesModel() {
	return this->m_pTestBenchPropertiesModel;
}
...
//	this->m_pTestBenchPropertiesModel = NULL;
	this->m_pTestBenchPropertiesModel.reset(); // Nothing shared
}
...
void TEditApplicationPropertiesFrame::createEditorEntry(UnicodeString name, UnicodeString value) {
	...
	bool propertyIsObsolete = (this->m_pTestBenchPropertiesModel->isObsoleteProperty(name));
...

{
	...
	this->m_pTestBenchPropertiesModel->setPropertyValue(KeyName,KeyValue);
