...
namespace bertlv {
	...
	/**
	  ...
	  * Note that some of the ASN.1 types can be encoded using either primitive or a constructed encoding at the option of the sender.
	...
	// From http://en.wikipedia.org/wiki/Basic_Encoding_Rules#BER_encoding
	/**
	  * Models the "class" value of a BERTLV encoded tag field
	  */
	enum e_BERTLVClass {
	   eBERTLVClass_ASN1 = 0x00 				// The type is native to ASN.1
	  ,eBERTLVClass_Application = 0x40 			// The type is only valid for one specific application
	  ,eBERTLVClass_Context_Specific = 0x80 	// Meaning of this type depends on the context (such as within a sequence, set or choice)
	  ,eBERTLVClass__Private = 0xC0 			// Defined in private specifications
	};

	// From http://en.wikipedia.org/wiki/Basic_Encoding_Rules#BER_encoding
	/**
	  * Bit 6 (P/C) states whether the content is primitive like an INTEGER or constructed, which means it again holds TLV values like a SET.
	  * P/C               bit 6
	  * Primitive             0
	  * Constructed           1
	  */
	enum e_BERTLVContentType {
		eBERTLVContentType_Primitive = 0x00
		,eBERTLVContentType_Constructed = 0x20
	};
	...
	public:
		 ...
		 */
//		static void safe_sprintf(c_DataRepresentationFramework::c_ISO_8859_1_String& buf, const c_DataRepresentationFramework::c_ISO_8859_1_String& format, int val, int maxchars);
// maxchars not used after refactoring of methid 101118/KoH
	...
	std::vector < const BerTlv<_Tag>* > BerTlvParser<_Tag>::parseBerTlv(const c_BertlvByteVector& bytes, const size_t length,const size_t offset) {
		std::vector<const BerTlv<_Tag>*> items;
		// check arguments
//		VALIDATE3(bytes.size() >= offset, "bytes.size()=%i <= offset=%i", bytes.size(), offset);
...
//			bool done = false;
//			// length can be 0 (ref. cdoOP_OPEN)
//			if (length == 0) {
//				done = true;
//			}
			bool done = (length == 0);
...
//	      ,length(BerTlvLengthItem((long)content->byteVector().size())), content(content) {
	...
	namespace asn1 {
		...
		enum e_ASN1Tag {
			...
			//ENUMERATED	P	10	A
			//EMBEDDED PDV	C	11	B
			,eASN1Tag_Primitive_UTF8STring = eBERTLVClass_ASN1 + eBERTLVContentType_Primitive + 0x0C//UTF8String	P/C	C
			//RELATIVE-OID	P	13	D
			//(reserved)	-	14	E
			//(reserved)	-	15	F
			,eASN1Tag_SEQUENCE = eBERTLVClass_ASN1 + eBERTLVContentType_Constructed + 0x10 //SEQUENCE and SEQUENCE OF	C	10
			,eASN1Tag_SET = eBERTLVClass_ASN1 + eBERTLVContentType_Constructed + 0x11 //SET and SET OF	C	17	11
